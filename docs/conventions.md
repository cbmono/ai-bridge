...
| 1 | [Retiring seed content is only reported](#1-retiring-content-is-asymmetric) | `/ai-bridge:init`, `/ai-bridge:welcome fix`, `RETIRED`, `seed/` |
...
| 4 | [A deletable capability is one file](#4-a-capability-some-deployments-must-not-have-should-be-one-deletable-file) | `docs/autonomy/AUTONOMY.md`, `commit-as.sh`, `/ai-bridge:welcome fix` |
...
| 17 | [An instruction is executable only if the agent *holds* the tool](#17-an-instruction-addressed-to-an-agent-is-executable-only-if-that-agent-holds-the-tool) | every agent body, `seed/CONVENTIONS.md`, `seed/CLAUDE.md` |
...
- **(a) Absence is the off switch, and the file is generated ROOT content — never a file under `plugin/`.** The writer rewrites it only when it already exists, `/ai-bridge:init` creates it on the **first stamp only** (`FIRST_STAMP`), and the renderer leaves a snapshot-less instance off the page entirely, with no placeholder. That is the `AWAITING.md` pattern, and putting the file under `plugin/` instead would inherit the `AUTONOMY.md` hazard directly above — machinery is re-linked unconditionally, so a per-instance `rm` comes back by itself.
...
`/ai-bridge:init` reports it with the exact replacement and **never edits `CLAUDE.md`**, which
...
nothing installed) and the PM's plan critic, not a feature. The bare `/ai-bridge:init <dir>` interface is unchanged
for the same reason: three live instances and `/ai-bridge:welcome fix` call it that way, so
...
directories it scans, with the tier names pinned separately against `/ai-bridge:init`'s own
...
`/ai-bridge:init` stamps, the hook ships with the `ai-bridge` plugin — so arming an instance
...
they never wrote. Doing it that way also keeps the whole feature out of `/ai-bridge:init` and
...
`/ai-bridge:init`'s `git rm --cached`. Three reasons the hook must not touch `log.md` itself:
...
- **(a) The vocabulary is pinned from BOTH sides, because a closed list leaks silently.** A
  ...
  **grants** must appear in the vocabulary (a new harness tool becomes real here at the
  ...
  shape-based: OKF's document types come from `seed/SCHEMA.md`'s own `type:` headings, so
...
- **(b) The scanned set is DERIVED, never listed.** Two paths were hardcoded, so the check
  could only ever look at the two someone remembered — and `seed/SCHEMA.md`'s
  ...
  **told to read it**, so that is what is derived: the `*.md` references in restricted agent
  bodies, resolved against `plugin/` then `seed/`. Adding a doc reference to an agent body
...
be checked from here. `seed/CLAUDE.md` is covered instead and `/ai-bridge:welcome fix` is the route for
...
machinery `/ai-bridge:init` re-linked unconditionally; since AI Bridge 2.0 it is a **plugin
...
themselves, and they are the expensive ones: a **new** file under `plugin/` reaches an
instance only when `/ai-bridge:init` runs again — measured twice in one week, `deny-destructive.sh`
...
installer ships or runs — `plugin/`, `seed/`, `config/`, `/ai-bridge:init`, `/ai-bridge:welcome fix`,
