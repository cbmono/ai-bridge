...
be checked from here. `plugin/seed/CLAUDE.md` is covered instead and `/ai-bridge:welcome fix` is the route for
...
`VERSION`), `config/`, `/ai-bridge:init`, `/ai-bridge:welcome fix` — which is *exactly* the union of the two path-scoped rule files' `paths:` globs
...
(*"always use the pm-loop"*) even contradict `plugin/seed/CLAUDE.md`, which says ad-hoc work must
