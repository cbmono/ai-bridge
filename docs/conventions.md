...
| 11 | [The board's four invariants](#11-the-cross-instance-board-is-two-scripts-and-one-deletable-generated-file) | `write-snapshot.sh`, `build-board.sh` |
...
**…and every piece of that is deliberate.** `write-snapshot.sh` derives `SNAPSHOT.json` at the bundle root from the schema-defined locations; `build-board.sh` renders one or more snapshots as a single self-contained HTML page. Four invariants.
...
Covered by `tests/snapshot.test.sh` (146 assertions, mostly negative).
