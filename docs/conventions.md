...
**A corpus rescore is a change detector, not a correctness proof.** All 35 PRs, both versions, identical snapshotted input, **0 of 35 exit codes changed** — for three rounds running. That is a null result about *reachability*: every behavioural change in the last two rounds is unreachable in this corpus, so the rescore gives **zero coverage of the new logic**. It shows the gate was not widened; the evidence that a route is closed is the constructed case and its passing control.
