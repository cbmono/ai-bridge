...
- **The tool-allowlist check is pinned from both sides, because a closed list leaks in the one direction nobody can see: silence.** A tool missing from the vocabulary is **invisible**, not "prose" — the check passes, which is indistinguishable from there being nothing to find. So every tool an agent **grants** must be in the vocabulary, and an unclassified backticked multi-word CamelCase name **fails**; the scanned set is **derived** from the docs agents are told to read, never a hardcoded list. A waiver's `(N)` budget is **not** what decides — a declared mention must **state the absence**, since a reword holds a count constant — and **`if` is not an absence cue**, because accepting it would bless the original defect. ([18](docs/conventions.md#18-the-tool-allowlist-check-is-pinned-from-both-sides-and-silence-is-a-failure))
