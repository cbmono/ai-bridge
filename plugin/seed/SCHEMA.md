...
| `board` | **no** — one instance, one answer, and `/ai-bridge:init` reads it from the tracked file at stamp time | on: `SNAPSHOT.json` is seeded, each tick renders `.ai-bridge/.board-live/board.html`, and a tick that changed something commits the tracked `/board.html` |
...
| `maxAgentMinutes` | **yes** — an hour of agent is a spend, and how long one machine tolerates is a per-machine call | **45**: past it an armed instance refuses the agent's next tool call with a `deny` that says to commit, push, open or update the PR and report. `Read`/`Grep`/`Glob` and a `git commit`, `git push` or `gh pr create\|edit\|view\|checks` stay allowed so that report is accurate; `0` (or anything not a positive integer) is off |
