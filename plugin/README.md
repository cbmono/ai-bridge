...
| `/dispatch [gap]` | The gated background loop. During the migration it delegates to the bundle's proven `/ai-bridge:dispatch` contract verbatim — no second implementation to drift. |
...
`/new-project`, `/close-project` and finally `/ai-bridge:dispatch`, which is `/dispatch` here. The
