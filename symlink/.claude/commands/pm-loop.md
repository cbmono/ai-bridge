...
line). Publishing was account-scoped and is deleted — `docs/pm-design.md#launcher-no-publish`.
...
4. **When `/pm-loop` re-fires from that wakeup:** if this session's own history says a
