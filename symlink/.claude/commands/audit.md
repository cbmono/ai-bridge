...
1. Read `instance.config.json`. **Resolve the auditor's model** the same way the PM
   ...
   alias via `models`; if those maps are absent it prints why on stderr, so report that
   line and then inherit the session model rather than dispatching on a guess.
