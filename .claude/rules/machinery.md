...
- **State every fallback, every time.** These docs are read inside instances whose config predates the key: `worktreeRoot` → `<reposRoot>/_wt`, `maxPrLoc` → 500, `maxAgentsInFlight` → 10, `PUSH_STATE_MAX` → 12, `boardInstances` absent → just this instance.
