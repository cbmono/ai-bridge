#!/usr/bin/env bash
#
# config-hardening.test.sh — the safety properties of the shipped config layer:
# `config/opinionated/settings.json`'s permission shapes, and the handful of properties in
# the shipped commands and agents that can be checked mechanically.
#
# WHY. These files arrived in this repo unchanged from elsewhere and are in daily use, so
# the failure mode is a fix regressing quietly on the next edit. Three groups:
#
...
#      Separately, this repo's convention allows only the simple pattern shapes
#      (`Bash(cmd:*)` and exact strings): a mid-pattern wildcard is documented but
#      fragile against flag re-ordering, redirects and env substitution, i.e. against
#      exactly the caller trying to evade it. The wildcard checker is itself exercised
#      against a synthetic bad pattern, so it cannot pass by never firing.
...
#      diffs, plans and other agents' findings out to subagents. That is the same
#      instruction/data boundary `show-awaiting.sh` keeps (see awaiting-queue.test.sh):
#      the payload is labelled DATA, wrapped in markers, and hostile text is REPORTED
#      rather than dropped — a hunk you cannot see is a hunk you cannot fix.
#
...
# shipped text — the same class as snapshot.test.sh's "no GNU-only regex escape has come
# back". They prove a specific regression has not returned; they do not prove the command
# behaves correctly at runtime. Several of the fixes in this area have no test at all and
# are honest prose only. Where a property spans one line of a JS template literal, it is
# checked within that line, because that is the unit the text is written in.
#
# Both tiers under `config/` are deletable by design, so an absent file is a SKIP.
#
# ok() compares actual to expected, in that argument order — this directory's convention.
set -uo pipefail

REPO="$(cd "$(dirname "$0")/.." && pwd)"
CMD="$REPO/config/opinionated/commands"
SET="$REPO/config/opinionated/settings.json"
...
if [ ! -f "$SET" ]; then
  echo "  SKIP: config/opinionated/settings.json absent (deletable tier)"
...
if [ ! -d "$CMD" ]; then
  echo "  SKIP: config/opinionated/commands absent (deletable tier)"
else
  ...
  # it is reported rather than obeyed (surfaced, not dropped — the awaiting-queue rule).
  ...
  # ------------------------------------------- acp scans before it stages
  # `git add -A` stages every non-ignored path, so a scan that runs afterwards is a scan
  # of an index that already holds the secret.
  # Ordinal, not textual proximity: step 1 must be the scan and the staging command must
  # be step 2. (Step 1 *mentions* `git add -A` to explain why it runs first, so a plain
  # "first mention" check would match the explanation.)
...
# ------------------------------------- the required tier: cleanup leaks are in scope
SCAN="$REPO/config/required/agents/deep-bug-scan.md"
if [ ! -f "$SCAN" ]; then
  echo "  SKIP: config/required/agents/deep-bug-scan.md absent"
else
