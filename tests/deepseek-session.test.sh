...
# `config/opinionated/scripts/deepseek-session.sh`.
#
...
#
# `config/opinionated` is a deletable tier, so an absent script is a SKIP, never a failure.
#
...
SRC="$REPO/config/opinionated/scripts/deepseek-session.sh"
...
if [ ! -f "$SRC" ]; then
  echo "SKIP: config/opinionated/scripts/deepseek-session.sh absent (deletable tier)"
  exit 0
fi
