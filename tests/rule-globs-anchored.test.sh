...
# "corrected" back by the next reader, so it is asserted here instead.
#
...
b=0; d=0; t=0
pats="$(awk '/^paths:/{p=1;next} /^---/{p=0} p && /^[[:space:]]*-[[:space:]]/{print}' "$tmp/.claude/rules/bad.md")"
while IFS= read -r line; do
  [ -n "$line" ] || continue
  pat="$(printf '%s' "$line" | sed -e 's/^[[:space:]]*-[[:space:]]*//' -e 's/^"//' -e 's/"$//')"
  t=$((t+1))
  case "$pat" in ./*) d=$((d+1));; /*) ;; *) b=$((b+1));; esac
done <<EOF
$pats
EOF
ok "checker parses a 3-pattern fixture"  "$t" 3
ok "checker flags the unanchored one"    "$b" 1
ok "checker flags the ./ one"            "$d" 1
