...
run_cfg() {  # <dest> <tpl> [extra args…] → exit code
  ...
  CLAUDE_CONFIG_DIR="$dest" bash "$tpl/plugin/scripts/init-bundle.sh" --config "$@" >"$TMP/out" 2>&1
  printf '%s' "$?"
}
...
CLAUDE_CONFIG_DIR="$D8" bash "$TPL/plugin/scripts/init-bundle.sh" "$I2" >"$TMP/out" 2>&1
...
bash "$TPL/plugin/scripts/init-bundle.sh" "$I3" >"$TMP/out" 2>&1
ok "bare TARGET still stamps an instance" "$(yn test -f "$I3/instance.config.json")" yes
I4="$TMP/inst-flag"; mkdir -p "$I4"
bash "$TPL/plugin/scripts/init-bundle.sh" --instance "$I4" >"$TMP/out" 2>&1
ok "--instance TARGET does the same"      "$(yn test -f "$I4/instance.config.json")" yes
bash "$TPL/plugin/scripts/init-bundle.sh" --config "$I4" >"$TMP/out" 2>&1
ok "--config with a target is refused"    "$?" 2
bash "$TPL/plugin/scripts/init-bundle.sh" --config --instance >"$TMP/out" 2>&1
...
bash "$TPL/plugin/scripts/init-bundle.sh" "$I5" >"$TMP/out" 2>&1
